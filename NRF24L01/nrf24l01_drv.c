...
返 回 值 ： 0 ---- 发送失败
			...
			0xFF ---- 达到最大重发次数，发送失败
*************************************************/
uint8_t NRF24L_SendPacket( const uint8_t *pData )
{
	uint8_t state = 0x70;
...
//	NRF_CE(LOW);									// CE拉低，使能24L01配置
	NRF24L_Write_Buf(pData, TX_PLOAD_WIDTH);		// 写数据到TX BUF,注意：长度必须与设置长度等长
	NRF_CE(HIGH);
	NRF24L_Delay_us(10);
...
返 回 值 ： 0 ---- 接收失败
			1 ---- 接收成功
*************************************************/
bit NRF24L_ReceivePacket( uint8_t *pData )
{
	uint8_t state = 0x70;
	...
	if(state & RX_OK)
	{
		NRF24L_Read_Buf(pData, RX_PLOAD_WIDTH);		// 读出RX BUF的数据,注意：长度必须与设置长度等长
		NRF24L_Clean_RX_Flush();
		NRF_CE(HIGH);
		NRF24L_Delay_us(10);
		NRF_CE(LOW);
		
		UART_SendString(pData, 32);
		LED7 ^= 1;
...
void NRF24L_Init(void)
{
	...
	const uint8_t rx_addr[5] = {0x34, 0x43, 0x10, 0x10, 0x01};
